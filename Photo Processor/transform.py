    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                ...
                #Everything is closer to being in the same color if it is decreased; different if it is increased
        ...
        for y in range(y_pixels):
            for c in range(num_channels):
                ...
                for x_i in range(max(0,x-neighbor_range), min(new_im.x_pixels-1, x+neighbor_range)+1):
                    #we do max(0,x-nighbor_range ) so that the if the value of x is negative than it is equal to zero
